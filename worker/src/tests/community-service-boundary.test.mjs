...
function memoryRepository() {
  ...
  return {
    ...
    async getTokenConnection(installIdHash) {
      const connection = connections.get(installIdHash);
      return connection ? { accessEnvelope: JSON.stringify(connection.accessEnvelope), workspaceName: connection.workspaceName } : null;
    },
...
test('a connected configured extension delivers a capture without commercial services', async () => {
  ...
  const app = createWorker({
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (String(url).endsWith('/oauth/token')) return new Response(JSON.stringify({ access_token: 'ntn_test', workspace_name: 'Community workspace' }), { status: 200 });
  ...
  const start = await app.fetch(new Request('https://api.example/v1/auth/notion/start', { method: 'POST', headers }));
  ...
  const removedPaths = [['/v1/', 'lic', 'ense'], ['/v1/', 'lic', 'ense/activate'], ['/v1/', 'usage', '/report'], ['/v1/webhooks/', 'le', 'mon']].map((parts) => parts.join(''));
