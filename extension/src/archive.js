...
function cardEditor(card) {
  ...
  if (card.truncated) { const warning = document.createElement('p'); warning.className = 'warning'; warning.textContent = 'Text truncated at 200,000 characters.'; article.append(warning); }
...
function selectCard(card) {
  ...
  $('#readerText').textContent = card.bodyText || 'This evidence has no captured text.';
...
$('#openFullReader').onclick = () => {
  $('#fullReaderText').textContent = $('#readerText').textContent;
