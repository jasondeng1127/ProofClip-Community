...
export function captureFeedback(result) {
  if (result?.ok) {
    const truncation = result?.record?.truncated ? ' Text truncated at 200,000 characters.' : '';
    const remaining = Number.isInteger(result?.remaining) ? ` ${result.remaining} work${result.remaining === 1 ? '' : 's'} left this month.` : '';
    return { text: `Evidence saved locally.${remaining} Open Archive to review or send it to Notion.${truncation}`, isError: false };
  }
