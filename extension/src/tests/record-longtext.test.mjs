...
test('a page above the retired 200,000-character ceiling preserves its exact tail', async () => {
  const longBody = `${'a'.repeat(200_001)}\nPROOFCLIP-LONG-PAGE-TAIL`;
  const record = await createEvidenceRecord({ title: 'Huge page', url: 'https://example.test/huge', bodyText: longBody }, 'body');
  assert.equal(record.truncated, false);
  assert.equal(record.bodyText, longBody);
  assert.equal(record.bodySha256, await sha256(longBody));
});
