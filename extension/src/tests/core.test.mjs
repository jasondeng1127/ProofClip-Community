...
test('a saved target Data Source is returned after a later state read', async () => {
  ...
  try {
    await saveState({
      settings: { dataSourceId: '01be583b-00d5-83d8-845f-0784db446a24', titleProperty: 'Name', urlProperty: 'URL' },
      archive: [],
      outbox: []
    });
    const reread = await getState();
    assert.equal(reread.settings.dataSourceId, '01be583b-00d5-83d8-845f-0784db446a24');
  } finally {
