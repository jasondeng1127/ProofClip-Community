...
The exporter exclusively reserves a new candidate directory before reading
source objects and rejects an existing candidate directory or sibling `.sha256`
sidecar before writing. The sidecar must contain the complete fingerprint and
the exact candidate basename. The verifier uses non-following directory-entry
checks and rejects candidate symlinks or other non-regular entries with
category/path findings only.
