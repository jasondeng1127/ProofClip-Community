...
test('OAuth exchange sends the Notion API version required by the provider', async () => {
  let request;
  const result = await exchangeAuthorizationCode({
    ...
    fetchImpl: async (url, options) => {
      request = { url, options };
      if (options.headers['Notion-Version'] !== '2026-03-11') return new Response(JSON.stringify({ message: 'Notion-Version is required' }), { status: 400 });
      return new Response(JSON.stringify({ access_token: 'ntn_test', refresh_token: 'nrt_test', workspace_id: 'workspace' }), { status: 200 });
    }
  });
