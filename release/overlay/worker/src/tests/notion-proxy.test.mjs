...
test('successful Notion page creation writes SENT in the create payload', async () => {
  const calls = [];
  const record = { title: 'Evidence', canonicalUrl: 'https://example.com', bodyText: 'body', capturedAt: '2026-08-09T00:00:00Z', mode: 'body', delivery: { status: 'PENDING' } };
  const target = {
    dataSourceId: '01be583b-00d5-83d8-845f-0784db446a24',
    fieldMappings: { title: 'Name', url: 'URL', deliveryStatus: 'Delivery status' },
    propertyTypes: { title: 'title', url: 'url', deliveryStatus: 'select' }
  };

  await writeCapture('ntn_test', record, target, async (url, options) => {
    calls.push({ url, options });
    return new Response(JSON.stringify({ id: 'page-sent', url: 'https://www.notion.so/page-sent' }), { status: 200 });
  });

  const createPayload = JSON.parse(calls[0].options.body);
  assert.equal(createPayload.properties['Delivery status'].select.name, 'SENT');
});

test('Notion provider failures become readable status-specific errors without provider details', async () => {
  const record = { title: 'Evidence', canonicalUrl: 'https://example.com', bodyText: 'body', delivery: { status: 'PENDING' } };
...
test('maps V1 evidence fields to compatible configured Notion properties', () => {
  ...
  assert.equal(payload.properties.Status.status.name, 'SENT');
});
...
test('setup settings survive extension storage merging and retain every mapped optional field for capture delivery', async () => {
  ...
  assert.equal(payload.properties['Delivery status'].select.name, 'SENT');
});
