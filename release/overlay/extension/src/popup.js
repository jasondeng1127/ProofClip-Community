...
import { getProofClipApiOrigin } from './core/proofclip-api.mjs';
...
function updateMappingControls() {
  ...
  $('#setupDataSource').disabled = !eligible;
...
const setupDataSourceButton = $('#setupDataSource');
setupDataSourceButton.onclick = async () => {
  const dataSourceId = $('#dataSourceId').value.trim();
  if (!canSaveTargetMapping(notionConnected, dataSourceId)) return renderActionFeedback(actionFeedback('notion'), failureFeedback('Choose a Notion Data Source before setting up ProofClip.'));
  await runButtonAction({
    button: setupDataSourceButton,
    feedback: actionFeedback('notion'),
    pendingLabel: 'Setting up ProofClip',
    successText: 'ProofClip is ready for this Data Source.',
    action: async () => {
      const result = requireOk(await send({ type: 'SETUP_DATA_SOURCE', dataSourceId }), 'ProofClip setup could not be completed.');
      currentSettings = result.settings;
      renderDataSources(dataSources, currentSettings.dataSourceId);
      renderTemplateOptions(currentSettings.templateId || 'buyer-account');
      renderTemplateMappings();
      return result;
    },
...
connectNotionButton.onclick = async () => {
  await runButtonAction({
    ...
    action: async () => requireOk(await send({ type: 'START_NOTION_CONNECTION' }), 'Notion authorization could not be opened.')
  });
};
...
// The privacy disclosure lives on the deployer-owned Worker; the link is
// filled from the configured Community API origin at runtime.
try {
  const privacyLink = document.querySelector('.privacy-link');
  if (privacyLink) privacyLink.href = `${getProofClipApiOrigin()}/privacy`;
} catch { /* keep the link inert until a valid Community origin is configured */ }
