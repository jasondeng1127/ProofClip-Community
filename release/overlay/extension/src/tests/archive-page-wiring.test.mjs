...
test('popup opens a local archive page and archive page uses only local archive commands', async () => {
  ...
  assert.match(archiveJs, /This previously saved record was truncated/);
...
test('archive batch feedback reports actual queued records instead of all failures', async () => {
  const archiveJs = await readFile(new URL('../archive.js', import.meta.url), 'utf8');
  assert.match(archiveJs, /result\.queued/);
  assert.match(archiveJs, /\$\{result\.queued\} failed record\(s\) are in Outbox/);
  assert.doesNotMatch(archiveJs, /result\.failed record\(s\) are in Outbox/);
});
