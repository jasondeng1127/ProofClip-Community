...
test('sends every selected archive record sequentially and continues after a failure', async () => {
  ...
  const result = await sendArchiveBatch(['card-1', 'card-2', 'card-3'], async (id) => {
    calls.push(id);
    return id === 'card-2' ? { ok: false, queued: true, error: 'Notion target unavailable.' } : { ok: true };
  });

  assert.deepEqual(calls, ['card-1', 'card-2', 'card-3']);
  assert.deepEqual(result, { total: 3, sent: 2, failed: 1, queued: 1 });
});

test('keeps failed and queued counts separate when a failed record is not persisted', async () => {
  const result = await sendArchiveBatch(['card-1', 'card-2'], async (id) => (
    id === 'card-1' ? { ok: false, queued: false, error: 'Invalid record.' } : { ok: false, queued: true, error: 'Notion unavailable.' }
  ));

  assert.deepEqual(result, { total: 2, sent: 0, failed: 2, queued: 1 });
});
...
test('ignores duplicate and empty archive ids before delivery', async () => {
  ...
  assert.deepEqual(result, { total: 2, sent: 2, failed: 0, queued: 0 });
});
...
test('continues the batch when one delivery handler throws unexpectedly', async () => {
  ...
  assert.deepEqual(calls, ['card-1', 'card-2']);
  assert.deepEqual(result, { total: 2, sent: 1, failed: 1, queued: 0 });
});
